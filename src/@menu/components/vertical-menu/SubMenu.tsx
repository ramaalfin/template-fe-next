...
const SubMenu: ForwardRefRenderFunction<HTMLLIElement, SubMenuProps> = (props, ref) => {
  ...
  const handleSlideToggle = (): void => {
    ...
    toggleOpenSubmenu?.({ level, menu, active, id })
  ...
  useEffect(() => {
    if (confirmUrlInChildren(children, pathname)) {
      openSubmenusRef?.current.push({ level, menu, active: true, id })
    } else {
      if (defaultOpen) {
        openSubmenusRef?.current.push({ level, menu, active: false, id })
      }
    }
    ...
    >
      ...
      >
        ...
        <StyledMenuLabel
          ...
          rootStyles={getSubMenuItemStyles('menu')}
